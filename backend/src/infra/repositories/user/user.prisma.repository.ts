...
import { UserEntityToUserPrismaModel } from './model/mappers/user-entity-to-user-prisma-model.mapper';
...
export class UserPrismaRepository extends UserGateway {
  ...
  public async create(user: User): Promise<void> {
    const aModel = UserEntityToUserPrismaModel.map(user);
    await prismaClient.user.create({ data: aModel });
  }
}
